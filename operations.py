...
async def endtimer(channel, client, wait):
    ...
    await channel.send("Loading everyone's words...")
    words = {}
    ...
    for word in sortedwords:
        ...
        if (points != 1):
            string += "s"
        if (onboard):
            if (spellcheck.priblink(word) == True):
        ...
        count += 1
        if (count % 15 == 0):
            totalwordsarr.append(totalwords)
            totalwords = ""
        # await channel.send(string)
        # await asyncio.sleep(1)
    totalwordsarr.append(totalwords)
    for word in totalwordsarr:
        print(totalwordsarr)
        await channel.send(word)
    await asyncio.sleep(1)
    # await channel.send(totalwords)
    # await asyncio.sleep(1)
...
#         client.usernames.remove(message.author)
class HelpOperation():
    def helpmessage(self):
...
class SessionOperation():
    ...
    async def run(self, message, content, client):
        if isinstance(message.channel, discord.channel.DMChannel):
           await message.channel.send("Boggle games cannot be started through DM. Please start a new game in a server channel.")
        else:
            client.scores = {}
            client.usernames = {}
