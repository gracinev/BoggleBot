    ...
    def makeImage(self):
        ...
        for line in self.board:
            ...
            for char in line:
                ...
                draw.rectangle(((x-90, y-90), (x+90, y+90)),
                               fill=(50, 139, 158))
                draw = ImageDraw.Draw(image)
                w, h = draw.textsize(char, font=font)
                draw = draw.text((x-(w/2), y-(h/2)), char,
                                 (235, 235, 235), font=font)
                x += 200
            y += 200
        image.save("img.png")

    def hasword(self, word):
        boardArr = np.concatenate(self.board, axis=0).ravel()
        lettersUsed = []
        for letter in word.upper():
            if letter in boardArr:
                np.delete(boardArr, np.where(boardArr == letter))
